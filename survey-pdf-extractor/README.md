...
- 誤読（7 と 9 など）が疑われるときは `--set-marker` で直せます。修正はキャッシュに
  保存されるので、次回以降は指定不要です

```bash
# 例: 101・102 ページの番号が 17 なのに 19 と読まれていた場合
python main.py --show-groups --set-marker 101=17 102=17
```

**典型的な誤読の見分け方**: あるグループがページ不足で、その直後のグループが
「飛び地」になっていたら、間のページが隣の番号として誤読されています。
（例: `17 → p97-100（4ページ）` ＋ `19 → p101,102,109-114（8ページ）`
なら、p101・p102 は 17 の誤読）
...
| `--set-marker 101=17 102=17` | 読み取った番号を手で修正する（`--show-groups` で警告が出たとき） |
